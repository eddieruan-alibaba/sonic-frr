...
Covers the PIC Phase 1 NHT event dplane path at the FRR side. Validation is
purely log-based (no FPM wire capture required):

  On every RNH state change that survives the `state_changed && !route_entry_queued`
  filter, `dplane_nht_event_update()` emits a zlog_info line:

      NHT_EVENT_UPDATE: rnh=<prefix> prev_prefix=<prefix> prev_nhg=<u32>
                       curr_prefix=<prefix> curr_nhg=<u32>

  The test drives topology changes on r2 and greps r1's zebra.log for that
  line, parsing the 5 fields.
...
The FPM-wire serialization (RTM_NEWNHTEVENT type 6000) lives in
`sonic-buildimage`'s `dplane_fpm_sonic.c` (Plan A Task 13) and is verified
separately at the fpmsyncd / sonic-mgmt layer.
"""
...
def test_nht_event_prev_only():
    """撤回 nexthop 对应路由后，应触发 curr_nhg=0 的 NHT event。"""
    tgen = get_topogen()
...
def test_nht_event_curr_only():
    """路由重新宣告后，应触发 prev_nhg=0, curr_nhg!=0 的 NHT event。"""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
...
def test_nht_event_prev_and_curr():
    """IGP 重收敛切换 NHG -> prev != 0, curr != 0."""
    pass


def test_no_nht_event_when_idle():
    """空闲期不应有新的 NHT event（sanity check）。"""
    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
