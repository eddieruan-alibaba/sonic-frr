...
| "已装" 统一语义 | `INSTALLED \|\| (zebra_nhg_fib_enabled && INSTALLED_FPM_ONLY)` |
| 清理路径 | 现有所有 `UNSET INSTALLED` 的点同步 `UNSET INSTALLED_FPM_ONLY` |
| 作用域守卫 | 所有读取处显式加 `zebra_nhg_fib_enabled &&` 守卫；社区默认行为零差异 |
...
### 5.3 读取点（统一加 RIBFIB 守卫）

所有"是否已装/不需重发/可触发 uninstall"的判断点，统一改成：
```c
CHECK_FLAG(.., INSTALLED) ||
(zebra_nhg_fib_enabled && CHECK_FLAG(.., INSTALLED_FPM_ONLY))
```
