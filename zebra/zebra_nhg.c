	...
	if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_INSTALLED) ||
		...
		case ZEBRA_DPLANE_REQUEST_FAILURE:
			flog_err(EC_ZEBRA_DP_DELETE_FAIL,
				 "Failed to uninstall Nexthop ID (%pNG) from the kernel", nhe);
			break;
		case ZEBRA_DPLANE_REQUEST_SUCCESS:
			UNSET_FLAG(nhe->flags, NEXTHOP_GROUP_INSTALLED);
			UNSET_FLAG(nhe->flags, NEXTHOP_GROUP_INSTALLED_FPM_ONLY);
...
void zebra_nhg_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (IS_ZEBRA_DEBUG_DPLANE_DETAIL || IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("Nexthop dplane ctx %p, op %s, nexthop ID (%u), result %s", ctx,
...
struct nhg_hash_entry *zebra_nhg_proto_del(uint32_t id, int type)
{
	...
	if (type != nhe->type) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("%s: id %u, type %s mismatch, sent by %s, ignoring", __func__,
				   id, zebra_route_string(nhe->type), zebra_route_string(type));
		return NULL;
	}
	...
	if (nhe->refcnt > 1) {
		if (IS_ZEBRA_DEBUG_NHG)
			zlog_debug("%s: %pNG, still being used by routes refcnt %u", __func__, nhe,
				   nhe->refcnt);
		return nhe;
	}

	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: deleted nhe %p (%pNG), vrf %d, type %s", __func__, nhe, nhe,
...
static void zebra_nhg_score_proto_entry(struct hash_bucket *bucket, void *arg)
{
	...
	if (nhe->type == iter->type && PROTO_OWNED(nhe)) {
		if (IS_ZEBRA_DEBUG_NHG_DETAIL)
			zlog_debug("%s: found nhe %p (%pNG), vrf %d, type %s after client disconnect",
...
unsigned long zebra_nhg_score_proto(int type)
{
	...
	for (ALL_LIST_ELEMENTS_RO(iter.found, ln, nhe)) {
		...
		if (!CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_PROTO_RELEASED))
			 ...
			 */
			if (IS_ZEBRA_DEBUG_RIB_DETAILED)
				zlog_debug("%s: nhe %u (%p) refcount %u already decremented in zread_nhg_del",
...
void zebra_interface_nhg_reinstall(struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: Installing interface %s associated NHGs into kernel", __func__,
			   ifp->name);
		...
		if (!nexthop_is_blackhole(nh) && ZEBRA_NHG_IS_SINGLETON(rb_node_dep->nhe)) {
			...
			frr_each_safe (nhg_connected_tree,
				...
				while (nhop_dependent &&
				       !nexthop_same_no_weight(nhop_dependent, nh))
